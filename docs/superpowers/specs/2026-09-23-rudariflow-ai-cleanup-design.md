...
- **Language kept against rules (found in the user's live test):** an Outlook rule "formal, complete sentences, German: Sie-Form" made all three models translate English dictations into German ("Done." became "Erledigt."). A prompt rule alone did not fix it. Now Whisper's detected language is named in the request ("The dictation is in English. Write the result in English, whatever the instructions say."), the system prompt says instructions never change the language, and the output guard rejects answers in another language (whatlang; when the two detections differ it decides between just those two languages and needs a clear result on both sides; under five words it does not judge). The test box, which has no audio, detects the language from the text. Benchmark with the user's sentences: all three models keep English and still write formal German for German dictations.
